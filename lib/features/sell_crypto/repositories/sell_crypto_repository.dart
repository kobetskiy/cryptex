...
abstract class SellCryptoRepositoryInterface {
...
class SellCryptoResponse {
  ...
  factory SellCryptoResponse.fromJson(Map<String, dynamic> json) {
    return SellCryptoResponse(
...
class SellCryptoRepository implements SellCryptoRepositoryInterface {
  final Dio _dio;

  SellCryptoRepository({Dio? dio})
      : _dio = dio ??
            Dio(
              BaseOptions(
  ...
  Future<SellCryptoResponse> sellCrypto({
    ...
    try {
      ...
      print('===========================================');
      print('=== SELL CRYPTO REQUEST ===');
      print('URL: ${_dio.options.baseUrl}/user/$userId/wallet/sell');
      print('Method: POST');
      ...
      if (wallet != null && wallet['amountOfCoins'] != null) {
        ...
        print('Coins in wallet after sell:');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (wallet != null && wallet['amountOfCoins'] != null) {
          final coins = wallet['amountOfCoins'] as List;
          final soldCoin = coins.firstWhere(
            (c) => c['name'] == coin,
            orElse: () => null,
          );
          if (soldCoin != null) {
            cryptoAmount = soldCoin['amount']?.toDouble();
          }
        }
